...
def _extract_license_expression_and_names_from_sbom(
    ...
    for item in extract_items_from_cdx_sbom(
        ...
        for entry in licenses_short:
            ...
            elif (lic_dict := entry.get("license", {})) and (lic_name := lic_dict.get("name", "")):
    ...
    expressions = sorted(set(lic_expressions))
...
def list_all_licenses(sbom_path: str, flict_simplify: bool = False) -> list[str]:
    ...
    return sorted(set(expressions + names))
...
def _craft_single_spdx_expression(licenses: list[str]) -> str:
